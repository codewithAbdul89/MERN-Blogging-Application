...
export const removeProfilePicture = asyncHandler(async (req, res) => {
  ...
  return res.status(200).json(
    new ApiResponse(200, "Profile picture removed successfully.", {
      user: updatedUser,
    }),
  );
});
...
export const sendDeleteAccountOtp = asyncHandler(async (req, res) => {
  ...
  const expiresAt = new Date(Date.now() + EMAIL_EXPIRY.DELETE_ACCOUNT_OTP);
...
export const deleteAccount = asyncHandler(async (req, res) => {
  ...
  const blogImagePublicIds = (
    await Blog.find({ author: userId }).distinct("featuredImage.public_id")
  ).filter(Boolean);

  if (blogImagePublicIds.length > 0) {
    await Promise.all(
      blogImagePublicIds.map((publicId) => deleteFile(publicId)),
    );
  }
