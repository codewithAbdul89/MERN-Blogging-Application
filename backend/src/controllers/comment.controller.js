...
export const deleteComment = asyncHandler(async (req, res) => {
  ...
  await Blog.findByIdAndUpdate(comment?.blog, {
    $inc: {
      commentsCount: -1 - comment.replyCount,
    },
  });
...
export const pinComment = asyncHandler(async (req, res) => {
  ...
  if (comment.status !== "VISIBLE") {
    throw new ApiError(400, "Only visible comments can be pinned.");
  }

  const blog = await Blog.findById(comment.blog);

  if (!blog.author.equals(req.user._id) && req.user.role !== "ADMIN") {
    throw new ApiError(403, "Forbidden. You cannot pin comments on this blog.");
  }
  ...
  if (comment.isPinned) {
    ...
    return res.status(200).json(
      new ApiResponse(200, "Comment unpinned successfully.", {
        isPinned: false,
      }),
    );
  }
  ...
  return res.status(200).json(
    new ApiResponse(200, "Comment pinned successfully.", {
      isPinned: true,
    }),
  );
});
...
//     throw new ApiError(400, "Comment already hidden.");
//   }
...
//   if (!isAdmin && !isBlogAuthor) {
//     throw new ApiError(403, "Forbidden. You cannot hide this comment.");
//   }
...
//   return res
//     .status(200)
//     .json(new ApiResponse(200, "Comment hidden successfully.", { comment }));
// });
...
//     throw new ApiError(403, "Forbidden , You cannot update this comment.");
//   }
